...
//  _debug->println("bmi270_init done");
  ...
  if (getAccelConfig(odr, range, bwp, perf)) {
    _acc_lsb_per_g = accelLsbPerG(range);
  }
  uint8_t noisePerf = 0;
  if (getGyroConfig(odr, range, bwp, noisePerf, perf)) {
    _gyr_lsb_per_dps = gyroLsbPerDps(range);
  }
...
int8_t BMI2_BMM1_Class::bmi2_accel_set_config(struct bmi2_dev *bmi2_dev)
{
  ...
  if (rslt == BMI2_OK)
  {
    ...
    if (rslt == BMI2_OK)
    {
      ...
      config.cfg.acc.bwp = BMI2_ACC_RES_AVG128;
...
bool BMI2_BMM1_Class::getAccelConfig(uint8_t& odr, uint8_t& range, uint8_t& bwp, uint8_t& filterPerf) {
  struct bmi2_sens_config cfg{};
  cfg.type = BMI2_ACCEL;
  const int8_t rslt = bmi2_get_sensor_config(&cfg, 1, &bmi2);
  if (rslt != BMI2_OK) return false;
  odr = cfg.cfg.acc.odr;
  range = cfg.cfg.acc.range;
  bwp = cfg.cfg.acc.bwp;
  filterPerf = cfg.cfg.acc.filter_perf;
  return true;
}

bool BMI2_BMM1_Class::getGyroConfig(uint8_t& odr, uint8_t& range, uint8_t& bwp, uint8_t& noisePerf, uint8_t& filterPerf) {
  struct bmi2_sens_config cfg{};
  cfg.type = BMI2_GYRO;
  const int8_t rslt = bmi2_get_sensor_config(&cfg, 1, &bmi2);
  if (rslt != BMI2_OK) return false;
  odr = cfg.cfg.gyr.odr;
  range = cfg.cfg.gyr.range;
  bwp = cfg.cfg.gyr.bwp;
  noisePerf = cfg.cfg.gyr.noise_perf;
  filterPerf = cfg.cfg.gyr.filter_perf;
  return true;
}

bool BMI2_BMM1_Class::setAccelConfig(uint8_t odr, uint8_t range, uint8_t bwp, uint8_t filterPerf) {
  struct bmi2_sens_config cfg{};
  cfg.type = BMI2_ACCEL;
  int8_t rslt = bmi2_get_sensor_config(&cfg, 1, &bmi2);
  if (rslt != BMI2_OK) return false;
  cfg.cfg.acc.odr = odr;
  cfg.cfg.acc.range = range;
  cfg.cfg.acc.bwp = bwp;
  cfg.cfg.acc.filter_perf = filterPerf;
  rslt = bmi2_set_sensor_config(&cfg, 1, &bmi2);
  if (rslt == BMI2_OK) _acc_lsb_per_g = accelLsbPerG(range);
  return rslt == BMI2_OK;
}

bool BMI2_BMM1_Class::setGyroConfig(uint8_t odr, uint8_t range, uint8_t bwp, uint8_t noisePerf, uint8_t filterPerf) {
  struct bmi2_sens_config cfg{};
  cfg.type = BMI2_GYRO;
  int8_t rslt = bmi2_get_sensor_config(&cfg, 1, &bmi2);
  if (rslt != BMI2_OK) return false;
  ...
  cfg.cfg.gyr.bwp = bwp;
  cfg.cfg.gyr.noise_perf = noisePerf;
  cfg.cfg.gyr.filter_perf = filterPerf;
  rslt = bmi2_set_sensor_config(&cfg, 1, &bmi2);
  if (rslt == BMI2_OK) _gyr_lsb_per_dps = gyroLsbPerDps(range);
  return rslt == BMI2_OK;
}
...
struct bmi2_sens_data sensor_data = { { 0 } };
   ...
   x = sensor_data.acc.x * 9.80665f / acc_lsb_per_g;

    y  = sensor_data.acc.y * 9.80665f / acc_lsb_per_g;

    z  = sensor_data.acc.z * 9.80665f / acc_lsb_per_g;
...
struct bmi2_sens_data sensor_data = { { 0 } };
    ...
    const float gyr_lsb_per_dps = _gyr_lsb_per_dps;
     ...
     x  = sensor_data.gyr.x / gyr_lsb_per_dps;

    y = sensor_data.gyr.y / gyr_lsb_per_dps;

    z = sensor_data.gyr.z / gyr_lsb_per_dps;
...
bool BMI2_BMM1_Class::readTemperatureC(float& tempC) {
  uint8_t buf[2] = {0, 0};
  // BMI270 temperature data registers: 0x22 (LSB), 0x23 (MSB).
  const int8_t rslt = bmi2_get_regs(0x22, buf, 2, &bmi2);
  if (rslt != BMI2_OK) return false;
  const int16_t raw = (int16_t)(((uint16_t)buf[1] << 8) | buf[0]);
  tempC = 23.0f + ((float)raw / 512.0f);
  return true;
}
