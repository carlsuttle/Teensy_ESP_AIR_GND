...
$log='c:\Users\dell\Platformio\esp32_crsf_telemetry\Teensy_ESP_AIR_GND\scripts\manual_replaybench_hang_'+(Get-Date).ToString('yyyyMMdd_HHmmss')+'.log'
'AIR+TEENSY manual replaybench repro' | Set-Content $log
...
try {
  Drain-Both $airCtx $teensy $log 1500
  $airCtx.Lines.Clear()
  ...
  $airCtx.Lines.Clear()
  Send-AirText $airCtx 'tapi replaybench 5000 50 48'
  $deadline=(Get-Date).AddSeconds(12); $gotFirst=$false
  while((Get-Date) -lt $deadline){ Read-ControlPort -Context $airCtx; Read-TextPort -Port $teensy -Label 'TEENSY' -LogPath $log; if(($airCtx.Lines | Select-Object -Last 20) -match '^TAPI REPLAYBENCH RESULT') { $gotFirst=$true; break }; Start-Sleep -Milliseconds 25 }
  if(-not $gotFirst){ throw 'first replaybench did not complete' }
  $airCtx.Lines.Clear()
  foreach($request in @(@{ category='state'; action='get' },@{ category='recording'; action='status' },@{ category='replay'; action='status' },@{ category='file'; action='storage_status' },@{ category='fusion'; action='get' })) { $resp = Invoke-AirControlRequest -Context $airCtx -Request $request -TimeoutMs 5000; Assert-ControlCompletedOk -Response $resp -Label ('manual {0}/{1}' -f $request.category,$request.action); Drain-Both $airCtx $teensy $log 250 }
  $airCtx.Lines.Clear()
  Send-AirText $airCtx 'tapi replaybench 5000 50 48'
  $deadline=(Get-Date).AddSeconds(18); $gotSecond=$false
  while((Get-Date) -lt $deadline){ Read-ControlPort -Context $airCtx; Read-TextPort -Port $teensy -Label 'TEENSY' -LogPath $log; if(($airCtx.Lines | Select-Object -Last 50) -match '^TAPI REPLAYBENCH RESULT') { $gotSecond=$true; break }; Start-Sleep -Milliseconds 25 }
  if(-not $gotSecond){ Write-ControlLog -Context $airCtx -Source 'INFO' -Text 'second replaybench appears hung; probing state'; foreach($request in @(@{ category='state'; action='get' },@{ category='recording'; action='status' },@{ category='replay'; action='status' },@{ category='file'; action='storage_status' },@{ category='fusion'; action='get' })) { try { $resp = Invoke-AirControlRequest -Context $airCtx -Request $request -TimeoutMs 4000; if($null -ne $resp.Final){ Write-ControlLog -Context $airCtx -Source 'POST' -Text ('{0}/{1} final={2}/{3}' -f $request.category,$request.action,$resp.Final.status,$resp.Final.code) } } catch { Write-ControlLog -Context $airCtx -Source 'POST' -Text ('{0}/{1} error={2}' -f $request.category,$request.action,$_.Exception.Message) }; Drain-Both $airCtx $teensy $log 300 }; Send-TeensyText $teensy 'showsource' $log; Drain-Both $airCtx $teensy $log 2500; throw 'second replaybench did not complete' }
