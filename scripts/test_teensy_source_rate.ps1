...
function Get-SourceSnapshot {
  ...
  $pattern = '^SOURCE CFG rate=(\d+)Hz period_us=(\d+) updates=(\d+) imu_acc_hz=([-\d.]+) imu_gyr_hz=([-\d.]+) supported=(.+)$'
  ...
  return [pscustomobject]@{
    ...
    Updates = [uint32]$match.Groups[3].Value
    ImuAccHz = [double]$match.Groups[4].Value
    ImuGyrHz = [double]$match.Groups[5].Value
    Supported = $match.Groups[6].Value
  }
}
...
try {
  ...
  foreach ($rate in $RatesHz) {
    Write-Host ("Testing {0} Hz..." -f $rate) -ForegroundColor Cyan
    ...
    if ($elapsedSec -le 0.0) {
      throw "Invalid elapsed time while measuring source rate"
    }

    $deltaUpdates = [double]([uint64]$snap2.Updates - [uint64]$snap1.Updates)
    $measuredFusionHz = $deltaUpdates / $elapsedSec
    ...
    $fusionPass = Test-Approx -Value $measuredFusionHz -Expected $rate -PercentTolerance 12.0
    ...
    $results.Add([pscustomobject]@{
      ...
      measured_fusion_hz = [Math]::Round($measuredFusionHz, 2)
      pass_source = $sourcePass
      pass_imu_acc = $imuAccPass
      pass_imu_gyr = $imuGyrPass
      pass_fusion = $fusionPass
      pass_all = ($sourcePass -and $imuAccPass -and $imuGyrPass -and $fusionPass)
    })
  }
  ...
  Write-Host ""
  Write-Host "Source Rate Verification Summary" -ForegroundColor Green
  $results |
    Select-Object target_hz, applied_hz, reported_rate_hz, imu_acc_hz, imu_gyr_hz, measured_fusion_hz, pass_all |
